...
<ul class="filter-links-container">
	...
	<li class="filter-link-item <?php print $active; ?>"><?php print l(t('All'), 
	  variable_get('fantorrent_legacy_list_path', 'legacy/list'), array('html' => true, 
    ...
    <?php foreach ($rows as $id => $row): ?>
      <?php if(!(date('Y',$view->result[$id]->node_created) == $date)) : ?>
        ...
        ?>	
        <li class="filter-link-item <?php print $active; ?>"><?php print l($date, variable_get('fantorrent_legacy_list_path', 'legacy/list') . '/' . $date, array('html' => true, 
        	'attributes' => array('class' => array('filter-link')))); ?></li>
      <?php endif; ?>
    <?php endforeach; ?>
</ul>